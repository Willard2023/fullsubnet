...
data_dir=../data/
corpus_name=data
train_data_dir=train_data_fsn_voicebank_master
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  ...
  # #### DNS-Challenge-master
  # # gen lst
  # /usr/bin/python3 -m speech_enhance.tools.gen_lst --dataset_dir ${corpus_name}/DNS2020/clean/ --output_lst ${train_data_dir}/clean.txt
  # /usr/bin/python3 -m speech_enhance.tools.gen_lst --dataset_dir ${corpus_name}/DNS2020/noise/ --output_lst ${train_data_dir}/noise.txt
  ...
  # # test_set=${corpus_name}/DNS-Challenge-master/datasets_16k/test_set
  # test_set=${corpus_name}/DNS2020/test_set
  # if [ ! -d ${test_set} ]; then
  ...
  #### voicebank
  /usr/bin/python3 -m speech_enhance.tools.gen_lst --dataset_dir ${corpus_name}/voicebank/clean_trainset_wav/ --output_lst ${train_data_dir}/clean_train.txt
  /usr/bin/python3 -m speech_enhance.tools.gen_lst --dataset_dir ${corpus_name}/voicebank/clean_testset_wav/ --output_lst ${train_data_dir}/clean_test.txt
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  CUDA_VISIBLE_DEVICES='2, 3' python -m speech_enhance.tools.train -C config/train_voicebank.toml -N 2
fi
